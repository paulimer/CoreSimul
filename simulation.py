...
def simulation(rseed, exp_coeff, min_delta, loss_rate, gain_rate, MATRIX, sub_rate, path_to_seq, sub, kappa, GC, L, COEFF, DELTA, coeff, path):
	sub = sub.split(",")
	coeff = float(coeff)
	DELTA = int(DELTA)
	COEFF = float(COEFF)
	...
	while nb <= NB:
		...
		for node in LIST: # Define the number of mutations and recombination events in the branch
			...
			mutations = np.random.poisson(LENGTH[node] * segment)
			i=1
			while i <= mutations:
				resu = node + "_m"
				MEGA.append(resu)
				i+=1
			recomb = np.random.poisson(LENGTH[node] * segment * COEFF)
			i=1
			while i <= recomb:
				resu = node + "_r"
				MEGA.append(resu)
			...
			if GAINS == "yes":
				gains = np.random.poisson(LENGTH[node] * segment * gain_rate)
				i=1
				while i <= gains:
					...
					i+=1
			if LOSSES == "yes":
				losses = np.random.poisson(LENGTH[node] * segment * loss_rate)
				i=1
				while i <= losses:
					resu = node + "_loss"
					MEGA.append(resu)
		...
		while mega_i < len(MEGA):
			...
			if event == "m":
				...
				if len(LIST)>1:
					...
					delta = np.random.exponential(DELTA)
					if delta < min_delta:
						delta = min_delta
					milou="-"
					TINTIN=0
...
if __name__ == "__main__":
	...
	min_delta = sys.argv[-14]
	exp_coeff = sys.argv[-15]
	rseed = sys.argv[-16]

	simulation(rseed, exp_coeff, min_delta, loss_rate, gain_rate, MATRIX, sub_rate, path_to_seq, sub, kappa, GC, L, COEFF, DELTA, coeff, path)
